...
my @ABBR_FULL_MAP = qw(d=weight-d lm=weight-l tm=weight-t w=weight-w
  g=weight-generation lex=weight-lex I=weight-i cslm=weight-cslm dlm=weight-dlm pp=weight-pp wt=weight-wt pb=weight-pb lex=weight-lex glm=weight-glm);
...
if ($___DECODER_FLAGS =~ /(^|\s)-(config|f) /
    ...
    || $___DECODER_FLAGS =~ /(^|\s)-(cslm-lmodel-file) /
