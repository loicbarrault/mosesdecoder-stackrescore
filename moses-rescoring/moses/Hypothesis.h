...
//	const Phrase			&m_targetPhrase; /*! target phrase being created at the current decoding step */
  ...
  float 						m_cslmScore;   /*! CSLM Score for search graph rescoring rescoring  */
...
// void PrintLMScores(const LMList &lmListInitial, const LMList	&lmListEnd) const;
  ...
  const float GetCSLMScore() const{
			return m_cslmScore;
  }
  ...
  inline void SetCSLMScore( float cslmScore ){
				m_cslmScore=cslmScore;
	}
