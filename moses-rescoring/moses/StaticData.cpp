...
bool StaticData::LoadData(Parameter *parameter)
{
  ...
  if( m_parameter->GetParam("cslm-lmodel-file").size() > 0 ){
				if (!LoadRescoringLanguageModels()) return false;
...
bool StaticData::LoadRescoringLanguageModels()
{
	
	if( m_parameter->GetParam("cslm-lmodel-file").size() > 0 ){
   // m_islmforrescoring = true;
		m_uselmrescoring = true;
  }

			vector<float> weight = Scan<float>(m_parameter->GetParam("weight-cslm"));
			m_lmrescoringweight = weight[0];
	...
	const vector<string> &lmVector = m_parameter->GetParam("cslm-lmodel-file");
	map<string,LanguageModel*> languageModelsLoaded;

	    for(size_t i=0; i<lmVector.size(); i++) {
      LanguageModel* lmresco = NULL;
      if (languageModelsLoaded.find(lmVector[i]) != languageModelsLoaded.end()) {
        lmresco = languageModelsLoaded[lmVector[i]]->Duplicate();
      } else {
        vector<string>  token   = Tokenize(lmVector[i]);
        if (token.size() != 4 && token.size() != 5 ) {                                                                                                                                          
          UserMessage::Add("Expected format 'LM-TYPE FACTOR-TYPE NGRAM-ORDER filePath [mapFilePath (only for IRSTLM)]'");                                                                       
      ...
      vector<FactorType>  factorTypes   = Tokenize<FactorType>(token[1], ",");                                                                                                                  
      size_t nGramOrder = Scan<int>(token[2]);                                                                                                                                                  
      m_lmrescoringNGramOrder = nGramOrder;                                                                                                                                                     
      string &languageModelFile = token[3];                                                                                                                                                     
        if (token.size() == 5) {                                                                                                                                                                
        ...
        IFVERBOSE(1)                                                                                                                                                                            
          PrintUserTime(string("Start loading Rescoring LanguageModel ") + languageModelFile);                                                                                                    
        
				m_rescorelm = new LanguageModelSRI();                                                                                                                                                     
        m_rescorelm->Load(languageModelFile,factorTypes[0],nGramOrder);
		}
	}

    IFVERBOSE(1)                                                                                                                                                                                              
       PrintUserTime("Finished loading Rescoring LanguageModels");                                                                                                                               
