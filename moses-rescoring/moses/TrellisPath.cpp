  ...
  :	m_prevEdgeChanged(NOT_FOUND)
{
  ...
  m_LMrescoring    = hypo->GetCSLMScore(); // The CSLM Score 
...
void TrellisPath::InitScore() {
  ...
  m_LMrescoring    = m_path[0]->GetWinningHypo()->GetCSLMScore(); // CSLM SCORE
//	std::cerr<<" TotalScore= "<<m_totalScore<<" Before score calc :      "<<m_scoreBreakdown<<endl;
  //calc score
