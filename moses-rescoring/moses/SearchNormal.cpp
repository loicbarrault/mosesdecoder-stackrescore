...
{
  VERBOSE(2, "Translating: " << m_source << endl);
  const StaticData &staticData = StaticData::Instance();

  if (m_initialTargetPhrase.GetSize() > 0) {
    VERBOSE(2, "Search extends partial output: " << m_initialTargetPhrase<<endl);
  }
  ...
  if (m_constraint) {
    VERBOSE(2, "Search constraint to output: " << *m_constraint<<endl);
  }
