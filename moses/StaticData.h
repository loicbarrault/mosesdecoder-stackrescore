...
//bougares
  bool   m_uselmrescoring; //! use LM Rescoring 	
...
//bougares
  ...
  //bougares ! load language model for rescoring

  bool LoadRescoringLanguageModels();
#ifdef HAVE_SYNLM
...
// bougares
  inline size_t GetLMRrescoringOrder() const{
        return m_lmrescoringNGramOrder;
  }
  float GetLMRescoringWeight() const{
        return m_lmrescoringweight;
  }
  bool UseLMRescoring() const {
	return m_uselmrescoring;
  }
  LanguageModelSRI * Getrescorelm() const{
	return m_rescorelm;	
  }
//bougares
